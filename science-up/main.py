...
class MMessageHandler(webapp2.RequestHandler):
    ...
    def post(self):
        ...
        new_wall_post = WallPost(sender=sender, message=message, subject=subject, space=space_key)
        new_wall_post.put() #puts object into database
        
        wall_posts = WallPost.query(WallPost.space==space_key).order(-WallPost.timestamp).fetch(100)
...
class MRosterHandler(webapp2.RequestHandler):
    def get(self):
        ...
        students = UserName.query(UserName.spaces==rs.key, UserName.status=='student')
        template_values['space_name'] = space_name
        template_values['space_key'] = space_key
        logout_url = users.create_logout_url('/')
        template_values ['logout_url']=logout_url
